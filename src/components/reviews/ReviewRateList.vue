...
<template>
  <div class="review__user-review" v-for="rate in rates" :key="rate.docID">
    <div class="review__user-img">
      <img :src="rate.user.image" v-if="rate.user.image.length > 0">
    ...
    <div class="review__user-text">
      <div>
        ...
        <span class="review__user-rate">
          <span class="review__user-rate-desc">{{ rate.rate }}</span>
        ...
        <span class="review__user-edit" v-if="rate.userID === currentUser">
          <button type="button" class="btn btn--link">Редактировать</button>
        </span>
      </div>
    </div>
  </div>
